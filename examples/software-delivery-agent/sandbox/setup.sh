...
# a warm sandbox without side effects. Installs no credentials — GH_TOKEN and
# BROWSER_USE_API_KEY / BU_CDP_URL / BU_CDP_WS are provided as environment
...
if ! command -v gh >/dev/null 2>&1; then
  ...
  install -d /etc/apt/keyrings 2>/dev/null || $SUDO install -d /etc/apt/keyrings
