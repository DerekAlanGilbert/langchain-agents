...
#
# Agent discovery is convention-based: any examples/<name>/ directory containing
# an agent.ts declaration is a deployable agent. Adding a new agent folder
...
#            and exit without deploying (used by CI and tests).
#   --all    Ignore the diff and select every discoverable agent.
#
# If the diff touches this script or any .github/workflows file, every agent is
# selected, since shared deploy tooling changes affect all of them.
set -euo pipefail
...
# An agent folder is examples/<name>/ containing an agent.ts declaration.
discover_all_agents() {
  find examples -mindepth 2 -maxdepth 2 -name agent.ts -type f 2>/dev/null |
